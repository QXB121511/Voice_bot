...
TTS_ORPHEUS_MODEL = "/data2/qinxb/AUDIO/RealtimeVoiceChat/models/orpheus-3b-0.1-ft-Q8_0-GGUF"

LLM_START_PROVIDER = "openai"
# LLM_START_PROVIDER = "ollama"
# LLM_START_MODEL = "qwen3:30b-a3b"
LLM_START_MODEL = "qwen3-32b"
# LLM_START_MODEL = "qwen1.5-72b-chat"
...
async def process_incoming_data_telephone(ws: WebSocket, app: FastAPI, incoming_chunks: asyncio.Queue, callbacks: 'TranscriptionCallbacks') -> None:
...
async def process_incoming_data(ws: WebSocket, app: FastAPI, incoming_chunks: asyncio.Queue, callbacks: 'TranscriptionCallbacks') -> None:
    ...
    metadata = {
        "client_sent_ms": timestamp_ms,                       # 客户端发送时间（毫秒）
        "client_sent": client_sent_ns,                        # 客户端发送时间（纳秒）
        "client_sent_formatted": format_timestamp_ns(...),    # 格式化的时间戳（例如 "2025-05-22 11:23:00.123"）
        "isTTSPlaying": bool(flags & 1),                      # TTS 是否在播放（从 flags 的最低位提取）
        "server_received": server_ns,                         # 服务器收到该包的时间（纳秒）
        "server_received_formatted": format_timestamp_ns(...),# 格式化的服务器接收时间
        "pcm": raw[8:],                                       # 剥去头部后的原始音频PCM数据（二进制）
    }
    """
    import ast
...
async def send_text_messages(ws: WebSocket, message_queue: asyncio.Queue) -> None:
    ...
    # data = {'type': 'partial_assistant_answer', 'content': "xxxxxxxxxxxxxx"}
    try:
        while True:
            ...
            #             "audioDataType": "raw",
            #             "sampleRate": 8000,
            #             "audioData": data.get("content") # Convert bytes to str
            #         }
            #     }
...
async def send_tts_chunks(ws: WebSocket, app: FastAPI, message_queue: asyncio.Queue, callbacks: 'TranscriptionCallbacks') -> None:
...
if __name__ == "__main__":
    ...
    if not USE_SSL:
        ...
        uvicorn.run("server:app", host="0.0.0.0", port=8889, log_config=None)
