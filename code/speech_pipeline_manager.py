...
base_url = "https://ai-api.knowdee.com/v1"
openai_key = "sk-C7kpDKgsnuSOGXQRD3C2De3c543d48DaA3B6D56cE900E33c"
...
# (Load system prompt)
try:
    with open("system_prompt_v6.txt", "r", encoding="utf-8") as f:
...
class SpeechPipelineManager:
    ...
    def __init__(
        ...
        self.llm_inference_thread = threading.Thread(target=self._llm_inference_worker_llm, name="LLMProcessingThread", daemon=True)
    ...
    def _llm_inference_worker(self):  
        """  
        修改后的LLM推理工作线程，不调用LLM，直接返回固定文本  
        """  
        logger.info("🗣️🧠 LLM Worker: Starting...")  
        while not self.shutdown_event.is_set():  
            try:  
                # 等待generator_ready_event信号  
                self.generator_ready_event.wait(timeout=0.1)  
                if not self.generator_ready_event.is_set():  
                    continue  # 超时，继续循环检查shutdown_event  
                ...
                if not self.running_generation:  
                    logger.debug("🗣️🧠⏸️ LLM Worker: No active generation, continuing...")  
                    continue  
                    
                # 获取当前生成的ID  
                gen_id = self.running_generation.id  
                logger.info(f"🗣️🧠🚀 [Gen {gen_id}] LLM Worker: Processing generation...")  
                
                # 标记LLM生成为活动状态  
                self.llm_generation_active = True  
                
                user_text = self.running_generation.text
                
                # 设置快速回答  
                self.running_generation.quick_answer = user_text
                
                # 设置事件，通知TTS可以开始合成  
                self.llm_answer_ready_event.set()  
                
                # 设置最终回答（空字符串，因为我们只使用quick_answer）  
                self.running_generation.final_answer = ""  
                
                # 标记快速回答已提供  
                self.running_generation.quick_answer_provided = True  
                
                # 模拟LLM生成器完成  
                self.running_generation.llm_generator = iter([])  # 空迭代器  
                
                # 标记LLM生成完成  
                self.running_generation.llm_generation_finished = True  
                
                # 重置LLM生成活动状态  
                self.llm_generation_active = False  
                
                logger.info(f"🗣️🧠✅ [Gen {gen_id}] LLM Worker: Fixed text generation complete.")  
                
            except Exception as e:  
                logger.exception(f"🗣️🧠💥 LLM Worker: Exception: {e}")  
                # 重置状态  
                self.llm_generation_active = False  
                # 清除事件  
                self.generator_ready_event.clear()  
                
        logger.info("🗣️🧠👋 LLM Worker: Shutting down.")

            
    def _llm_inference_worker_llm(self):
        """
...
# ========================================================================================================
                            # # 在这里中断，不继续处理完整的LLM响应  
                            # current_gen.llm_finished = True  # 标记LLM生成已完成  
                            # current_gen.llm_finished_event.set()  # 设置完成事件  
                            break
        ...
        with self.check_abort_lock:
            if self.running_generation:
                ...
                if self.running_generation.abortion_started:
                    ...
                    # =======================================这里默认7s
                    self.abort_generation(wait_for_completion=wait_for_finish, timeout=5.0, reason=f"check_abort found different text ({abort_reason})")
    ...
    def _tts_quick_inference_worker(self):
        ...
        while not self.shutdown_event.is_set():
            ...
            wait_timeout = 10.0 # Example timeout
    ...
    def process_abort_generation(self):
        ...
        with self.abort_lock:
            ...
            if is_tts_quick_potentially_active:
                ...
                stopped = self.stop_tts_quick_finished_event.wait(timeout=3.0) # Wait for TTS worker
                if stopped:
            ...
            if is_tts_final_potentially_active:
                ...
                stopped = self.stop_tts_final_finished_event.wait(timeout=3.0) # Wait for TTS worker
                if stopped:
    ...
    def abort_generation(self, wait_for_completion: bool = False, timeout: float = 7.0, reason: str = ""):
        ...
        logger.warning("🛑 [DEBUG-ENTER] 进入 abort_generation 方法")

        if self.shutdown_event.is_set():
            logger.warning("🗣️🔌 Shutdown in progress, ignoring abort request.")
            logger.warning("🛑 [DEBUG-EXIT] 因系统关闭跳过中止流程")
            return
        ...
        logger.warning("🛑 [DEBUG-PRE] 即将调用 process_abort_generation()")
        self.process_abort_generation()
        logger.warning("🛑 [DEBUG-POST] process_abort_generation() 已返回")

        # Optionally wait for completion
        if wait_for_completion:
            logger.info(f"🗣️🛑⏳ Waiting for abort completion (timeout={timeout}s)...")
            logger.warning(f"🛑 [DEBUG-EVENT-STATE] 等待终止完成")
            ...
            # 无论是否超时都释放阻塞
            logger.warning("🛑 [DEBUG-BLOCK-RELEASE] 正在设置 abort_block_event")
            self.abort_block_event.set()
            logger.warning(f"🛑 [DEBUG-BLOCK-STATE] abort_block_event设置后状态: {self.abort_block_event.is_set()}")

        logger.warning("🛑 [DEBUG-EXIT] 退出 abort_generation 方法")
...
#   ======================================================这里的timeout为17s
        self.abort_generation(wait_for_completion=True, timeout=10.0, reason="reset") # Ensure clean slate
        self.history = []
...
#   ==============================================默认为13
        self.abort_generation(wait_for_completion=True, timeout=13.0, reason="shutdown")
